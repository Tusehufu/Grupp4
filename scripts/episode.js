document.addEventListener('DOMContentLoaded', function () {
    ...
    (async () => {
        ...
        console.log('Episodes from API:', episodesFromAPI); // Lägg till denna logg

        let episodesFromLocalStorage = JSON.parse(localStorage.getItem('episodes')) || [];
        console.log('Episodes from LocalStorage:', episodesFromLocalStorage); // Lägg till denna logg

        // Kombinera avsnitten från API:et och local storage
    ...
    async function addEpisodeToFrontPage(episode) {
        ...
        // Kontrollera om avsnittet är sparad i localstorage och lägg till uppdateringsknappen endast för dessa avsnitt
        ...
        // Skapa en uppdateringsknapp för varje avsnitt om det finns i local storage
        if (isInLocalStorage) {
            ...
            updateEpisodeButton.addEventListener("click", () => {
                // Öppna modalen för att uppdatera avsnittet och fyll formuläret med aktuell avsnittsdata
    ...
    function showSingleEpisode(episode) {
        ...
        // Anropa calculateSeasonAndEpisode för att bestämma säsongen och avsnittsnumret
    ...
    document.getElementById('episodeForm').addEventListener('submit', function (event) {
        ...
        // Generera ett unikt ID för det nya avsnittet
        ...
        // Dölj modalen efter skapandet av det nya avsnittet
        const modal = document.getElementById("newEpisodeModal");
        const modalInstance = new bootstrap.Modal(modal);
        modalInstance.hide();

        // Rendera om avsnitten på sidan för att visa det nya avsnittet
        location.reload();
    });
    ...
    function openUpdateModal(episode) {
        // Fyll i formuläret med avsnittets befintliga data
        ...
        // Sätt de gamla värdena i de gömda fälten
...
// Uppdatera ett avsnitt i localStorage för att ändra säsong, avsnitt och releasedate
function updateEpisode(oldTitle, oldSeasonNumber, oldEpisodeNumber, newTitle, newSeasonNumber, newEpisodeNumber, releaseDate) {
    const episodes = JSON.parse(localStorage.getItem('episodes')) || []; // Hämta avsnitt från localStorage
    ...
    if (index === -1) {
        ...
        // Uppdatera avsnittet med nya värden
        episodes[index] = {
            title: newTitle,
        ...
        // Rendera om avsnitten på sidan för att visa uppdateringen
        location.reload();
    }
}


// Händelselyssnare för att uppdatera ett avsnitt
document.getElementById("updateEpisodeForm").addEventListener("submit", function (event) {
    event.preventDefault(); // Förhindra formulärinsändning
    ...
    // Rensa inmatningsfälten efter uppdatering för att undvika att behöva klicka två gånger
